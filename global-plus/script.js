...
function getGlobalCountry(){
  ...
  if(q === 'AR') { location.replace('../plus/app/'); return 'US'; }
...
if(window.resetBtn){ resetBtn.onclick = () => {
  if(confirm('¿Restaurar accesos personalizados, orden de secciones y recargar la maqueta?')){ localStorage.removeItem(LS_CUSTOM_TILES); localStorage.removeItem(LS_GROUPS_STATE); location.reload(); }
}; }
...
function initPlusAbTest(){
  ...
  if(wa){
    ...
    wa.textContent = (ACTIVE_PRESET.lang?.plusBenefits || 'Ver ventajas Plus');
  }
  btn.onclick = () => {
    try{
...
function applyGlobalPreset(){
  ...
  if(sel){
    ...
    sel.onchange = () => {
      if(sel.value === 'AR') { location.href = '../plus/app/'; return; }
  ...
  const customDialogH = document.getElementById('customDialogTitle'); if(customDialogH) customDialogH.textContent = l.addDialogTitle;
}
applyGlobalPreset();
